...
struct Point {
    ...
    bool operator==(const Point& other) const {
        return std::abs(x - other.x) < 1e-6f && std::abs(y - other.y) < EPS;
    }
...
std::vector<Polygon> createPolygons(std::vector<Node>& subject_extended,
   ...
   int number_of_polygon = 0;
    ...
    for (size_t i = 0; i < subject_extended.size(); i++) {
        if (subject_extended[i].type == INTERSECTION && subject_extended[i].isEntry && !visited[i]) {
            Polygon currentPoly;
            size_t currentIdx = i;
            bool inSubject = true;//
            do {//
                if (inSubject) {//
                    while (true) {
                        ...
                        if (subject_extended[currentIdx].type == INTERSECTION && !subject_extended[currentIdx].isEntry) {
                    ...
                    while (true) {
                        ...
                        if (clip_extended[currentIdx].type == INTERSECTION && clip_extended[currentIdx].isEntry) {
                            // Find matching point in subject polygon
                            for (size_t j = 0; j < subject_extended.size(); j++) {
                                if (subject_extended[j].type == INTERSECTION &&
                                    clip_extended[currentIdx].point == subject_extended[j].point) {
                                    currentIdx = j;
                                    break;
                                }
                            }
            ...
            if (currentPoly.vertices.size() >= 3) {
                ...
                std::cout << "Polygon " << ++number_of_polygon << " has " << cleaned.size() << " vertices" << std::endl;
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        subject.draw(window, sf::Color(255, 100, 100, 100));  // Semi-transparent red
        clip.draw(window, sf::Color(100, 100, 255, 100));     // Semi-transparent blue
